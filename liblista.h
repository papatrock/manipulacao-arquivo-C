#ifndef _LIBlista_t_H
#define _LIBlista_t_H
#define LINESIZE 1024


struct bicicleta{
    char  name[LINESIZE];
    char  date[LINESIZE];
    double distancia;
    double veloMed;
    double veloMax;
    double HrMedio;
    double HrMax;
    double cadenciaMed;
    double subidaAcu;

};
typedef struct bicicleta bicicleta_t;

struct dados{
    char date[LINESIZE];
    double speed;
    double maxSpeed;
    double timestamp;
    double speedxTimestamp;
    double somaTempo;
    double heartRate;
    double heartRatexTimestamp;
    double hearRateMax;
    double distance;
    double cadence;
    double cadencexTimestamp;
    double altitudeIni;
    double altitude;
};typedef struct dados dados_t;


struct nodo_l {
    bicicleta_t *elemento;       /* lista de numeros inteiros */
    struct nodo_l *prox;  /* ponteiro para o proximo   */
};
typedef struct nodo_l nodo_l_t;

struct nodoDados {
    dados_t *elemento;       
    struct nodoDados *prox;  
};
typedef struct nodoDados nodoDados_t;

struct lista {
    nodo_l_t* ini;      /* ponteiro para o inicio da lista */
    int tamanho;        /* numero de elementos na lista    */
};
typedef struct lista lista_t;

struct nodoChar{
    char nome[LINESIZE+1];
    struct nodoChar *prox;
};
typedef struct nodoChar nodoChar_t;

struct listaChar{
    nodoChar_t *ini;
};
typedef struct listaChar listaChar_t;

/*
 * Cria uma lista vazia e a retorna, se falhar retorna NULL.
*/
lista_t* lista_cria ();

listaChar_t *listaCriaChar();

int pedalMaisLongo(lista_t *lista);

int pedalMaisCurto(lista_t *lista);

nodo_l_t* cria_nodo(bicicleta_t *elemento);

nodoChar_t *criaNodoChar(char nome[LINESIZE+1]);

/*
 * Remove todos os elementos da lista, libera espaco e retorna NULL.
*/
lista_t* lista_destroi (lista_t* l);

listaChar_t *listaDestroiChar(listaChar_t *lista);

/*
 * Retorna 1 se a lista esta vazia e 0 caso contrario.
*/
int lista_vazia (lista_t* l);

/*
 * Retorna o tamanho da lista, isto eh, o numero de elementos presentes nela.
*/
int lista_tamanho (lista_t* l);

/*
 * Insere o elemento no inicio da lista.
 * Retorna 1 se a operacao foi bem sucedida e 0 caso contrario.
*/
int lista_insere_inicio (lista_t* l, bicicleta_t *elemento);

/*
 * Insere o elemento no final da lista.
 * Retorna 1 se a operacao foi bem sucedida e 0 caso contrario.
*/
int lista_insere_fim (lista_t* l, bicicleta_t *elemento);

int listaInsereNome (listaChar_t* lista,char *nome);

/*
 * Insere o elemento na lista garantindo ordenacao em ordem crescente.
 * Retorna 1 se a operacao foi bem sucedida e 0 caso contrario.
*/
int lista_insere_ordenado (lista_t* l, bicicleta_t *elemento);

/*
 * Remove o elemento do inicio da lista e o retorna em 'elemento'.
 * Retorna 1 se a operacao foi bem sucedida e 0 caso contrario.
*/
int lista_retira_inicio (lista_t* l, bicicleta_t *elemento);

/*
 * Remove o elemento do final da lista e o retorna em 'elemento'.
 * Retorna 1 se a operacao foi bem sucedida e 0 caso contrario.
*/
int lista_retira_fim (lista_t* l, bicicleta_t *elemento);

bicicleta_t *lista_retorna_elemento (lista_t *lista, char *nomeBici);

int lista_atualiza_elemento(lista_t *lista,bicicleta_t *bicicleta);

/*
 * Remove o elemento 'elemento' caso ele exista na lista.
 * Retorna 1 se a operacao foi bem sucedida e 0 caso contrario.
 * Se o elemento nao for encontrado na lista tambem retorna 0.
*/
int lista_retira_elemento (lista_t *l, bicicleta_t *elemento);

/*
 * Retorna 1 se o elemento existe na lista e 0 caso contrario.
*/
int lista_pertence (listaChar_t* lista,char *nomeBic);
/*
 * Imprime a lista, do inicio ate o fim, este por ultimo, sem espaco no final.
 * Quando a lista estiver vazia, nao imprime nada, nem um \n.
 * Esta funcao eh somente para facilitar teus testes.
 * Normalmente ela nao existe nas implementacoes de um TAD lista.
*/

#endif